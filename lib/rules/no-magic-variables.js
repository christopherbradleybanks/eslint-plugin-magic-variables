...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      avoidUse: "Avoid using variable names that are numeric consider refactoring '{{name}}' to be a more descriptive variable ",
    }
  },
    ...
    return {
      Identifier(node) { 
          if (isNumeric(node)) {
              ...
              let value = get(node, 'parent.init.value', get(node,'parent.init.argument.value', null))
              value = modifier ? value * modifier : value
              context.report({
                node: node,
                messageId: value !== null ? "avoidName" : "avoidUse",
                data: {
                    name,
                    value,
                },
